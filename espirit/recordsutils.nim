...
proc info* (record: MWPathgrid): string =
    var ppoints = ""
    for pi, pp in record.ppoint.pairs:
      ppoints.add("\n" & fmt"      - {pi + 1}) X: {pp.x} Y: {pp.y} Z: {pp.z} | Connections: {pp.con_count}")
      if pp.flags == 1: ppoints.add(" | Autogenerated")
    result = fmt"""
    Path point count: {record.data.ppoint_count}
    Path points:{ppoints}
    """

proc info* (record: MWDialogueTopic): string =
    const ops = [
      '0': "=",
      '1': "!=",
      '2': ">",
    ...
    var je = "" # journal entry (special)
    var rt = "" # result
    var fv = "\n    Variables: None" # function/variable
    if record.qname  == 1: je.add("\n[Quest Name]")
    elif record.qfin == 1: je.add("\n[Quest Finished]")
    elif record.qres == 1: je.add("\n[Quest Restarts]")
    if record.rest != "":  rt.add("\nResult:\n" & record.rest)
    for v in record.fvstr:
      # v[0] parsing:
      # [0]   - index (1-5)
      # [1]   - type (1-C), see MWDialogueTopicType
      # [2-3] - details (skip)
      # [4]   - operator
      # [rem] - name
      fv = fv.replace(" None", "") # won't be done if 'fvstr' is empty
      fv.add("\n" & fmt"      - Index: {v[0][0]} | Type: {MWDialogueTopicType(v[0][1])} | Variable: {v[0][5..len(v[0])-1]} {ops[v[0][4]]}")
      if v[1] > 0:
        fv.add(fmt" {v[1]}")
      elif v[2] > 0.0:
        fv.add(fmt" {v[2]}")
    ...
    result = fmt"""
    ID:   {record.id}
    Type: {MWDialogueType(record.data.kind)}
    Disposition/Journal Index: {record.data.disp_ji}{je}{fv}
    ---
...
proc info* (record: MWDialogue): string =
    var tpc = ""
    for _, tp in record.topics.pairs:
      tpc.add("\n    ---o---\n" & tp.info)
    result = fmt"""
    Name: {record.name}
    Type: {MWDialogueType(record.kind)}
    =====
    Topics:{tpc}
    """